...
import prisma from "./db"; // ✅ on query direct la DB (plus propre que listReservations)
...
function mergeRanges(ranges) {
  ...
  for (const r of a) {
    ...
    if (new Date(r.start) <= new Date(last.end)) {
      last.end = new Date(Math.max(+new Date(last.end), +new Date(r.end)));
    } else {
      out.push({ ...r });
    }
  }
...
// 2) Réservations faites sur TON site (via la DB)
// ✅ IMPORTANT : on ne remonte que les réservations "actives"
// - PAID / CONFIRMED
// - + PENDING non expirées (expiresAt > now)
  ...
  const rows = await prisma.reservation.findMany({
    where: {
      ...
      OR: [
        ...
        // pending seulement si pas expirée
