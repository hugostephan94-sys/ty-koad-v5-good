...
export default function CalendarSelectable({
  ...
  const today = useMemo(() => { const t=new Date(); t.setHours(12,0,0,0); return t; }, []);
  ...
  useEffect(()=>{
    ...
    (async ()=>{
      ...
      try {
        ...
        if(!ignore){ setBusy(new Set(data.bookedDates||[])); }
      } finally {
  ...
  /* ==== Sélection / règles ==== */
  function clearAll(){ setCheckIn(""); setCheckOut(""); }
  function resetAfter(ci){ setCheckIn(ci); setCheckOut(""); }
  ...
  function isStart(d){ return checkIn  && iso(d)===checkIn; }
  function isEnd(d){   return checkOut && iso(d)===checkOut; }
    ...
    <div className={`relative z-0 ${className || ""}`}>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
        <div className="text-sm text-slate-700">
          {checkIn ? (
            <>
              ...
              {nights>0 && !minOk && (
                <span className="text-amber-700 ml-1">(min {chalet.minNights})</span>
              )}
              {nights>0 && minOk && (
                <span className="ml-1">
                  • total{" "}
        ...
        <div className="flex items-center gap-3 text-xs">
          {chaletId==="C2" && (
            <>
              <span className="inline-flex items-center rounded-full bg-emerald-600/10 text-emerald-800 px-2 py-0.5 border border-emerald-300">
                Dim-jeu 130 €
              </span>
              <span className="inline-flex items-center rounded-full bg-amber-500/10 text-amber-800 px-2 py-0.5 border border-amber-300">
                Ven-sam 150 €
              </span>
            </>
          )}
          <button
            type="button"
            onClick={clearAll}
            className="px-2 py-1 rounded-lg border border-slate-300 text-slate-700 hover:bg-slate-50"
      ...
      <div className="flex items-center justify-between mb-3">
        <button
          ...
          className={`px-2 py-1 rounded-lg border border-slate-300 ${prevDisabled ? "opacity-40 cursor-not-allowed" : "hover:bg-slate-50"}`}
          aria-label="Mois précédent"
        >
          ‹
        </button>
        <div className="text-sm text-slate-700">{CHALETS[chaletId].name}</div>
        <button
      ...
      <div className="mt-3 text-[11px] text-slate-600">
        <span
          ...
          style={{backgroundImage:"repeating-linear-gradient(135deg, rgba(239,68,68,.36) 0, rgba(239,68,68,.36) 10px, transparent 10px, transparent 20px)"}}
        />
...
/* ---------- Un mois ---------- */
function Month({ date, busy, today, checkIn, checkOut, minNights, onPick, inRange, isStart, isEnd }){
  ...
  return (
    <div className="bg-white/95 border border-slate-200 rounded-3xl p-3 shadow-sm hover:shadow-md transition">
      <div className="text-center font-semibold capitalize text-slate-800">
        {title}
      </div>
        ...
        {days.map((d,idx)=>{
          ...
          return (
            <button
              ...
              disabled={disabled}
              className={[
                "relative h-9 rounded-lg text-[12px] flex items-center justify-center border",
                inMonth ? weekendTint : "bg-slate-50 text-slate-400",
                "border-slate-200 text-slate-800",
                disabled ? "opacity-60 cursor-not-allowed" : "hover:bg-slate-50",
                (startSel || endSel) ? "ring-2 ring-emerald-500 bg-emerald-50 font-semibold" : "",
                inSel ? "bg-emerald-100/40" : "",
              ].join(" ")}
              style={isBusy ? {
                backgroundImage: "repeating-linear-gradient(135deg, rgba(239,68,68,.36) 0, rgba(239,68,68,.36) 10px, transparent 10px, transparent 20px)"
              } : (tooShort ? {
                backgroundImage: "repeating-linear-gradient(45deg, rgba(16,185,129,.22) 0, rgba(16,185,129,.22) 6px, transparent 6px, transparent 12px)"
              } : undefined)}
              aria-label={key}
              title={
                isBusy ? "Indisponible"
                : isPast ? "Passé"
                : tooShort ? `Minimum ${minNights} nuits`
                : "Sélectionner"
              }
            >
              {d.getDate()}
