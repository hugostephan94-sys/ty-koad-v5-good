...
export default function BookingWidget({
  ...
  useEffect(() => {
    if (!ci || !co) {
      setMsg("Choisissez vos dates.");
      return;
    }
    if (pastError) {
      setMsg("Dates passées non autorisées.");
      return;
    }
    if (hasConflict) {
      setMsg("Ces dates ne sont plus disponibles.");
      return;
    }
    if (tooShort) {
      setMsg(
        `Séjour minimum : ${chalet.minNights} nuit${
  ...
  if (chalet?.externalOnly) {
    return (
      <aside className="bg-white rounded-3xl shadow-sm border border-stone-200 p-5 sm:p-6 text-sm">
        <div className="inline-flex px-2.5 py-0.5 rounded-full text-[11px] font-medium bg-emerald-100 text-emerald-900">
        ...
        <p className="mt-2 text-sm text-stone-600">
          Ce logement se réserve actuellement{" "}
          <strong>exclusivement</strong> sur Booking.com. Tarifs indicatifs :{" "}
          <b>130 €</b> (dim–jeu) • <b>150 €</b> (ven–sam).
        </p>
        <a
          href={chalet.bookingUrl || "#"}
          target="_blank"
          rel="noreferrer"
          className="mt-4 inline-flex w-full sm:w-auto items-center justify-center px-4 py-2.5 rounded-xl bg-emerald-700 hover:bg-emerald-800 text-white text-sm font-medium shadow-sm transition"
        >
          Voir sur Booking
        </a>
      </aside>
  ...
  return (
    <aside className="bg-white rounded-3xl shadow-sm border border-stone-200 p-5 sm:p-6 text-sm space-y-4">
      ...
      {!selected && (
        <div>
          ...
          <select
            className="mt-1 w-full rounded-xl border border-stone-300 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500"
            value={activeId}
      ...
      {msg && (
        <div
          className={`mt-1 text-xs sm:text-sm ${
            loading
              ? "text-stone-500"
              : msg.startsWith("Choisissez")
              ? "text-stone-600"
              : "text-red-700"
          }`}
        >
          {loading ? "Vérification…" : msg}
        </div>
      )}
      ...
      <button
        ...
        disabled={!canReserve}
        className={`mt-3 inline-flex items-center justify-center w-full px-4 py-2.5 rounded-xl text-sm font-medium shadow-sm transition ${
          canReserve
